...
// Copyright (C) 2018 The CrystalOS Project
//
...
func init() {
	android.RegisterModuleType("crystal_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = crystalExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = crystalExpandVariables(ctx, dep_file)
	...
	cmd := crystalExpandVariables(ctx, String(g.properties.Cmd))
