function __print_crystal_functions_help() {
cat <<EOF
Additional CrystalOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- crystalgerrit:   A Git wrapper that fetches/pushes patch from/to CrystalOS Gerrit Review.
- crystalrebase:   Rebase a Gerrit change and push it again.
- crystalremote:   Add git remote for CrystalOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for CrystalOS Github.
...
function Crystalize()
{
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "No such item in Crystalize menu. Try 'lunch'"
        return 1
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/crystal-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.crystal.device | grep -q "$CRYSTAL_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $CRYSTAL_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function crystalremote()
{
    ...
    git remote rm crystal 2> /dev/null
    ...
    local CRYSTAL_USER=$(git config --get review.review.crystalos.org.username)
    if [ -z "$CRYSTAL_USER" ]
    then
        git remote add crystal ssh://review.crystalos.org:29418/$PFX$PROJECT
    else
        git remote add crystal ssh://$CRYSTAL_USER@review.crystalos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'crystal' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/CrystalOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.crystal.device | grep -q "$CRYSTAL_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        crystalremote
        git push crystal HEAD:refs/heads/'$1'
    fi
    '
}

function crystalgerrit() {
    ...
    local user=`git config --get review.review.crystalos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "crystalgerrit" ]; then
...
function crystalrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CrystalOS Gerrit Rebase Usage: "
        echo "      crystalrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.crystalos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.crystal.device | grep -q "$CRYSTAL_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/crystal/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CRYSTAL_FIXUP_COMMON_OUT ]; then
